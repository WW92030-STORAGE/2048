...
class AI2048 {
    ...
    bool operator<(AI2048& o) {
        std::vector<double> v1 = {emptytile, rough, matsum};
        std::vector<double> v2 = {o.emptytile, o.rough, o.matsum};
        return v1 < v2;
    }
    
    AI2048() { }
    AI2048(double em, double ro, double ma) {
        emptytile = em;
        rough = ro;
        matsum = ma;
    }
    ...
    double getOneDirectionScore(Game2048 game) {
		...
		double retval = emptytile * empties + rough * roughness + matsum * matrixSum(game);
...
bool operator<(const AI2048& t, const AI2048& o) {
    std::vector<double> v1 = {t.emptytile, t.rough, t.matsum,};
    std::vector<double> v2 = {o.emptytile, o.rough, o.matsum};
    return v1 < v2;
}
...
AI2048 randomAI(double radius = 4096) {
    return AI2048(radius * randrad(), radius * randrad(), radius * randrad());
}

AI2048 mutate(AI2048 ai, double radius = 16) {
    AI2048 res(ai);
    int blah = rand() % 32;
    if (blah == 3) res.emptytile = radius * randrad();
	if (blah == 8) res.rough = radius * randrad();
	if (blah == 9) res.matsum = radius * randrad();
    return res;
}
